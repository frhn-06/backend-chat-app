...
const conversationController = {
    async findAll(req:IReqUser, res:Response) {
        try {
            ...
            if(!userId || !isValidObjectId(userId)) return response.unauthorize(res);
            ...
            response.error(res, error, "failed to find all conversations")
        }        
    },
    ...
    async findById(req:IReqUser, res:Response) {
        try {
            ...
            response.error(res, error, "failed to find a conversations")
        }        
    },
    ...
    async findByParticipants(req: IReqUser, res:Response) {
        try {
            ...
            if( !isValidObjectId(userId) || !isValidObjectId(targetId)) {
                return response.notFound(res, "participants is not found");
            }

            const sameId = userId === targetId;
            if(sameId) return response.error(res, new Error("duplicated id"), "id is same");
            ...
            response.error(res, error, "failed to find a conversations by participants")
        }
    }
}
